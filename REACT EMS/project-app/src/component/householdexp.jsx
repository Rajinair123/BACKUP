...
dayjs.extend(isSameOrBefore)
  ...
  const setFiltereddata = householdexpStore((state)=>state.setFiltereddata)
...
return(
        <div className="flex-col  flex items-center   ">
          <div className=''>
            <button className='bg-gray-500  hover:bg-gray-700 mb-25 text-white font-bold py-2 px-4 border m-5 border-gray-700 rounded'>Total expenses</button>
   ...
   onChange={()=>{dropdownChangeHandler()}} >
       ...
       <table className="mb-20 shadow-lg justify-center table-fixed m-10 ">
           <thead>
           ...
           <tbody>
                { householdexpense.map((h)=>(
                   <tr key={h._id}> 
                       ...
                       <td className="border px-1 py-2"><Link to={`/primarymember/householdexpForm/${h._id}`}> {h.household.name}
                            </Link></td> 
                       <td className="border px-1 py-2">{h.expensetype.name}</td>
                     <td className="border px-1 py-2">{h.paymentDetails[0].amount}, <br /> {h.paymentDetails[0].method}</td>  
           ...
           <Link to ="/primarymember/householdexpForm"><button className="text-white bg-gradient-to-r mb-15 from-blue-400 via-bblue-500 to-blue-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">
  +</button></Link>
